...
Dark Theme with Simplified Interface
"""
...
CATEGORY_COLORS = {
    "breaking_change":       "#FF6B6B",  # Red
    "deprecation_notice":    "#FFA94D",  # Orange
    "schema_change":         "#FFD93D",  # Yellow
    "security_notice":       "#FF6B8A",  # Pink
    "new_endpoint":          "#51CF66",  # Green
    "policy_change":         "#4ECDC4",  # Teal
    "regional_availability": "#74C0FC",  # Blue
    "informational":         "#ADB5BD",  # Grey
...
def build_excel_file(feed, new_items):
    ...
    for idx, item in enumerate(new_items, 1):
        ...
        color = CATEGORY_COLORS.get(classification['category'], "FFFFFF").replace('#', '')
    ...
    for cat, count in cat_count.most_common():
        ...
        color = CATEGORY_COLORS.get(cat, "FFFFFF").replace('#', '')
    ...
    for idx, entry in enumerate(feed.entries, 1):
        ...
        color = CATEGORY_COLORS.get(classification['category'], "FFFFFF").replace('#', '')
        ws_high.cell(row=row, column=4).fill = PatternFill("solid", start_color=f"FF{color}")
        ...
        ws_high.cell(row=row, column=6).font = Font(bold=True, color="FFFF0000")
...
def main():
    ...
    with col2:
        st.markdown("<h1 style='text-align: center;'> Amazon SP-API RSS Monitor</h1>", unsafe_allow_html=True)
        st.markdown("<p style='text-align: center; color: #8B949E;'>Automated Changelog Monitoring with Classification</p>", unsafe_allow_html=True)
    ...
    with col_left:
        # Fetch button
        if st.button("📥 Fetch Latest Feed", use_container_width=True):
            with st.spinner("Fetching RSS feed from Amazon..."):
                ...
                if feed and feed.entries:
                    ...
                    st.session_state.excel_data = build_excel_file(feed, new_items)
    ...
    with col_right:
        # Download Excel button - always visible when data is ready
        if st.session_state.excel_ready and st.session_state.excel_data:
            st.download_button(
            ...
            st.caption("📋 Excel contains 4 sheets: New Updates | All Items | Summary | Critical Items")
    ...
    if st.session_state.feed_data:
            ...
            if new_items:
                for idx, item in enumerate(new_items, 1):
                    ...
                    with st.expander(f"{idx}. {item['title'][:80]}...", expanded=(idx <= 3)):
                        ...
                        with col_b:
                            st.markdown(f"**Category:** <span style='background-color:{color};padding:4px 12px;border-radius:4px;font-weight:bold;color:#000000;'>{classification['category']}</span>", unsafe_allow_html=True)
                        ...
                        st.write(f"**Description:** {item['description'][:400]}...")
            else:
        ...
        # Welcome message
        st.info("👈 Click **'Fetch Latest Feed'** to get started!")
        
        st.markdown("""
        <div style="background-color: #1A1E26; padding: 20px; border-radius: 8px; border: 1px solid #2D333B; margin-top: 20px;">
            <h3 style="color: #FAFAFA;">🚀 Features</h3>
            <ul style="color: #ADB5BD;">
                <li><strong style="color: #FAFAFA;">Automated RSS Fetching</strong> - Pulls the latest Amazon SP-API changelog</li>
                <li><strong style="color: #FAFAFA;">Smart Classification</strong> - Categorizes each update into 9 categories</li>
                <li><strong style="color: #FAFAFA;">BP Impact Rating</strong> - Rates items by business relevance (High/Med/Low/None)</li>
                <li><strong style="color: #FAFAFA;">Excel Export</strong> - One-click download of categorized data with 4 sheets</li>
                <li><strong style="color: #FAFAFA;">State Tracking</strong> - Remembers what you've already seen</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
